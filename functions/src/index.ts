...
exports.creacionPedido = functions.firestore.document('pedidos/{pedidoId}').onCreate((snapshot, context)=>{
    ...
    const total_prendas:number = CalcularTotal(nuevo_pedido);
    let dia;
    let temp = {};
...
exports.edicionPedido = functions.firestore.document('pedidos/{pedidoId}').onUpdate((change)=>{
    ...
    const fecha = new Date(nuevo_pedido.fechaVenta - offsetTime);
    const total_prendas_nuevo:number = CalcularTotal(nuevo_pedido);
    const total_prendas_anterior:number = CalcularTotal(anterior_pedido);
    let dia;
    let temp = {}; 
    let caso = 0;
...
function CalcularTotal(pedido):number{
